    ...
    def get_type(self, spec):
        if "PrimitiveType" in spec and spec["PrimitiveType"] is not None:
            if spec["PrimitiveType"] in primitive_type_converters:
                return (
                    f'{primitive_type_converters[spec["PrimitiveType"]].__name__}()',
                    "property",
                )
            return (
                f'None # Unsupported primitive type "{spec["PrimitiveType"]}"',
                "property",
            )
...
class TerraformResourceTypeDefinition(TypeDefinition):
    def __init__(self, name, tf_name, spec, tf_spec):
        ...
        self.property_names = generate_best_name_matches(
            sorted(spec["Properties"].keys()), possible_names, snake_case_with_abbreviation_fix, score_cutoff=90,
        )
        self.attribute_names = generate_best_name_matches(
            sorted(self.cf_attrs), self.tf_attrs, snake_case_with_abbreviation_fix, score_cutoff=90,
        )
...
def main():
    ...
    for namespace, file_definitions in definitions.items():
        ...
        with open(f"{directory}/generated-spec/{namespace}.py", "w", encoding="utf-8") as spec_file:
